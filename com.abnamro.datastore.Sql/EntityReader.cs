...
{
    internal class EntityReader<TData,TEntity> : IDataRow
    {
        private readonly IDataReader _dataReader;
        private readonly IDictionary<string, int> _fieldMap;

        internal EntityReader(IDataReader dataReader)
        {
        ...
        internal IEnumerable<TEntity> ReadEntities(IDataMapper<TData, TEntity> dataMapper)
        {
            if (dataMapper == default(IDataMapper<TData, TEntity>)) throw new ArgumentNullException(nameof(dataMapper));
            while (_dataReader.Read())
            {
                yield return dataMapper.MapData(ToData(this));
            }
        }


        private TData ToData(IDataRow dataRow)
        {
            if (!(dataRow is TData)) throw new ArgumentException($"Invalid {nameof(dataRow)} return type {typeof(TData).Name}. Expected type {typeof(IDataRow).Name}.", nameof(dataRow));

            return (TData)dataRow;
        }
