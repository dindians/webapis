...
using com.abnamro.webapisInternal.iis;
...
namespace com.abnamro.webapisInternal.iis
{
    public class OwinStartup
    {
        ...
        public void Configuration(IAppBuilder appBuilder)
        {
            ...
            appBuilder.ConfigureWebApis(new WebApisStartOptions(new DeviceUserAuthorizer(maxLogonAttemtpsAllowed), oAuthTokenEndpointPath, oAuthAccessTokenExpirePeriodInMinutes:oAuthAccessTokenExpirePeriodInMinutes, useNLogTracer: true, traceName: $"{nameof(webapisInternal)}.{nameof(iis)}"), AppSettings.TryGetStringValue(ApiRoutePrefix, out string apiRoutePrefix) ? apiRoutePrefix : default(string), requireHttps:false);
        }
    }
}